...
public class OR_NOTG extends Problem implements SimpleProblemForm, MatchT {
	...
	public void evaluate(final EvolutionState state, final Individual ind, final int subpopulation,
		...
		for (int i = 0; i < (intVectorIndividual.genome.length - 1); i = i + 2) {
			...
			subbqb.add(new TermQuery(new Term(IndexInfoStaticG.FIELD_CONTENTS, word0)), BooleanClause.Occur.SHOULD);
			subbqb.add(new TermQuery(new Term(IndexInfoStaticG.FIELD_CONTENTS, word1)), BooleanClause.Occur.MUST_NOT);
		...
		def rawfitness  = (F1train * (words.size()/4)) / ((tree +1) * (tree +1) * (tree +1)) ;
		// +length
		// +duplicates
		// +tree
